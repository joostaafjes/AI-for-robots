...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        if turning_angle >= 0.001:
            ...
            result.x = cx + radius * sin(self.orientation + turning_angle)
            result.y = cy - radius * cos(self.orientation + turning_angle)
            result.orientation = (self.orientation + turning_angle) % (2 * pi)
        else:
            result.x += distance * cos(self.orientation)
            result.y += distance * sin(self.orientation)
            result.orientation = (self.orientation + turning_angle) % (2 * pi)
